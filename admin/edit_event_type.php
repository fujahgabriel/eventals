...
$event = $eventQuery->getEventTypes($id);
?>
<div class="w-full flex flex-wrap">
    ...
    <div class="w-full  md:mx-40 md:my-20 flex flex-col">
        ...
        <div class="flex flex-col justify-center md:justify-start my-auto pt-10 md:pt-15 px-8 md:px-24 lg:px-32">
            <p class="text-center text-2xl">Update Event Type - <?=$event[0]["title"]?></p>
            <div id="message"></div>
            <form class="flex flex-col pt-3 md:pt-8" id="updateEventTypeForm" method="POST">

                <div class="flex flex-col pt-4">
                    <label for="title" class="text-lg">Event Type Title</label>
                    <input type="text" name="title" id="title" value="<?=$event[0]["title"]?>" class="shadow appearance-none border rounded w-full py-4 px-3 text-gray-700 mt-1 leading-tight focus:outline-blue-400 focus:shadow-outline" required>
                </div>

               
               <input type="hidden" id="id" value="<?= base64_decode($_GET["id"])?>" name="id">
